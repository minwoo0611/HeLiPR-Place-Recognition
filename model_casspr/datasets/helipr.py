...
# Modified by: Minwoo Jung (SNU)

# Dataset wrapper for HeLiPR laser scans dataset
...
class HeLiPRDataset(Dataset):
    """
    Dataset wrapper for HeLiPR laser scans dataset from PointNetVLAD project.
    """
    ...
    def load_pc(self, filename):
        ...
        # pc[:, :3] = 100 * pc[:, :3]  # scale to meters
        pc = pc[np.linalg.norm(pc[:, :3], axis=1) < self.max_distance]
        pc = pc[:, :3]  # remove intensity
        if pc.size == 0:
            print(filename)
...
class RemoveRandomPoints:
    ...
    def __call__(self, e):
        ...
        # Create a tensor of zeros with the same shape as e[mask]
...
if __name__ == '__main__':
    ...
    query_filename = 'helipr_test_queries_frame_5m_baseline.pickle'

    my_dataset = HeLiPRDataset()
