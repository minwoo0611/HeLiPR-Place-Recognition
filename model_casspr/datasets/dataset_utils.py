...
from datasets.helipr import HeLiPRDataset, TrainTransform, TrainSetTransform
...
def make_datasets(params: MinkLocParams, debug=False):
    ...
    datasets['train'] = HeLiPRDataset(params.dataset_folder, params.train_file, params.num_points,
                                          params.max_distance, train_transform,
                                          set_transform=train_set_transform, max_elems=max_elems)
    val_transform = None

    datasets['val'] = HeLiPRDataset(params.dataset_folder, params.val_file,
...
def make_eval_dataset(params: MinkLocParams):
    ...
    dataset = HeLiPRDataset(params.dataset_folder, params.test_file, params.num_points, params.max_distance,
                                transform=None)

    return dataset


def make_collate_fn(dataset: HeLiPRDataset, version, dataset_name, mink_quantization_size=None, include_pnt=False, pnt2s=False, num_points=None):
            ...
            if version == 'MinkLoc3D':
                ...
                for e in batch:
                    ...
                    if include_pnt2s:
                        ...
                        # print(torch.nn.functional.pad(spherical_e, (0, 0, 0, padlen), "constant", 0).shape)
...
def make_dataloaders(params: MinkLocParams, debug=False):
    ...
    if 'val' in datasets:
        ...
        val_collate_fn = make_collate_fn(datasets['val'],  params.model_params.version, params.dataset_name,
                                         params.model_params.mink_quantization_size, num_points=params.num_points,
                                       include_pnt=include_pnt,
                                       pnt2s=pnt2s)
...
def to_spherical(points, dataset_name):
    ...
    for point in points:
        ...
        # HeLiPR scans are built from a 2D scanner.
